...
const Observer = Class({
  ...
  initialize() {
    ...
    this.on("select", tab => {
      ...
      if (selected !== tab) {
        ...
        if (tab) {
          this[selected] = tab;
          emit(this, 'activate', this[selected]);
        }
      }
    });
    ...
    windowObserver.on("open", chromeWindow => {
      if (isBrowser(chromeWindow)) {
        this.observeWindow(chromeWindow);
      }
    });
    ...
    windowObserver.on("close", chromeWindow => {
      if (isBrowser(chromeWindow)) {
        ...
        this.ignoreWindow(chromeWindow);
      }
    });
  ...
  observeWindow(chromeWindow) {
    this.observe(getTabContainer(chromeWindow));
  },
  ignoreWindow(chromeWindow) {
    this.ignore(getTabContainer(chromeWindow));
  },
  /**
