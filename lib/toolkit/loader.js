...
const Loader = iced(function Loader(options) {
  ...
  // Allows to overloading mapping through `commonjs.path.component` preferences
  let branch = prefService.getBranch('commonjs.path');
  let paths = branch.getChildList('', {}).reduce(function (result, name) {
    let path = name.substr(1).replace('.', '/');
    if (path) path += '/';
    let uri = branch.getCharPref(name);
    let resourcesURI = ioService.newURI(uri, null, null);
    let resName = 'commonjs.path' + name;
    resourceHandler.setSubstitution(resName, resourcesURI);
    result[path] = 'resource://' + resName + '/';
    return result;
  }, paths);
  // Set the default path if not provided or overlayed.
  if (!paths['']) paths[''] = 'resource://gre/modules/commonjs/';
